...
const Home = props => {
  ...
  const regionParams = props.route.params.region;

  // useEffect(() => {
  //   animateMap(regionParams);
  // }, [regionParams]);
  ...
  useEffect(() => {
    handleUserLocation(regionParams);
  }, [regionParams]);

  useEffect(() => {}, []);

  const handleUserLocation = (params) => {
