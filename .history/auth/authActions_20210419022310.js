...
export const signup = async (email, password) => {
  ...
  await fetch(
    `https://n2-mobile-2dbf3-default-rtdb.firebaseio.com/${resData.localId}/user.json?auth=${resData.idToken}`,
    {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
  ...
  await fetch(
    `https://n2-mobile-2dbf3-default-rtdb.firebaseio.com/${resData.localId}/locations.json?auth=${resData.idToken}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export const changePassword = async (
  ...
  alert(`Password changed successfully! ${JSON.stringify(resData)}`);
};
...
export const changeNameSurname = async (myName, mySurName, token, userId) => {
  ...
  const resData = await response.json();
  alert(`Name-Surname changed successfully as ${myName} ${mySurName}... Pull to refresh your profile.`);
};
